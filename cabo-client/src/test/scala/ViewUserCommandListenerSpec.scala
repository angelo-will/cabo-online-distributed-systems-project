...
import view.gamephase.actors.GameViewUserCommandListener
  ...
  "During Game view listener" must {
    "send message card selected" when {
      "the method is called" in {
        val duringGameActionListener: IGameViewUserCommandListener = GameViewUserCommandListener(testProbe.ref)
        val index = 2
