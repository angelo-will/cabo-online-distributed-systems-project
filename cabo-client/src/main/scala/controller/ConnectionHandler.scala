...
object ConnectionHandler:
  ...
  private def normalBehavior(parent: ActorRef[Message], playerReferences: List[PlayerInLobby]): Behavior[InternalCommand] = {
    Behaviors.receive { (ctx, msg) =>
      msg match {
        case MemberEventAdapter(event) => {
          event match {
            case MemberRemoved(member, previousStatus) => {
              ctx.log.warn(s"Member removed: ${member.address}, previous status: $previousStatus")
            ...
            case genericEvent => {
              ctx.log.info(s"Other member event:) $genericEvent")
              Behaviors.same
            }
          }
        }
        case UpdateList(playerList) => {
    ...
    //        //todo - decide if wait for the player to reconnect or not
  ...
  def apply[T <: MemberEvent](parent: ActorRef[Message])(using ct: ClassTag[T]): Behavior[InternalCommand] = Behaviors.setup { ctx =>
    ...
    val adapter = ctx.messageAdapter[MemberEvent](MemberEventAdapter.apply)
    ...
    //    )
    //todo - check if the list of player could be a variable argument (could be done by adding a message that launches the behavior only when we have the first player)
